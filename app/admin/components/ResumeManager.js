...
export default function ResumeManager() {
    ...
    // Only allow data:application/pdf (generated by this app) or http/https URLs.
    const sanitizeResumeUrl = (url) => {
        ...
        try {
            ...
            // Invalid URL, fall through to return null below
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const sanitizedDownloadURL = sanitizeResumeUrl(downloadURL);
            if (!sanitizedDownloadURL) {
                setStatusMessage({ type: 'error', text: 'Please enter a valid resume link (http/https) or upload a PDF file.' });
                setUploading(false);
                return;
            }
            ...
            const result = await saveSettings('profile', {
                resumeUrl: sanitizedDownloadURL,
                resumeUpdatedAt: new Date().toISOString(),
                resumeFileName: file ? file.name : 'External Link'
            });

            if (result.success) {
                setCurrentResume(sanitizedDownloadURL);
                setFile(null);
                                ...
                                <div className={styles.itemInfo} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                    ...
                                    <div>
                                        ...
                                        <span className={styles.itemMeta} style={{ wordBreak: 'break-all', display: 'block', maxWidth: '100%' }}>
                                            {currentResume && currentResume.startsWith('data:') ? 'Stored in Database (PDF)' : 'External Link'}
                                        </span>
                                        {sanitizeResumeUrl(currentResume) && (
                                            <a
                                                href={sanitizeResumeUrl(currentResume)}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                style={{ color: '#ffd700', textDecoration: 'underline', fontSize: '0.9rem' }}
                                                onClick={(e) => {
                                                    if (currentResume && currentResume.startsWith('data:application/pdf')) {
                                                        e.preventDefault();
                                                        try {
