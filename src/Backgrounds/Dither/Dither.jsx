...
function DitheredWaves({
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (enableMouseInteraction) {
      // Smooth mouse interpolation for buttery smooth animation
      const lerpFactor = Math.min(delta * 8, 1); // Adjust multiplier for smoothness (8 = responsive, lower = smoother)
      
      mouseRef.current.lerp(targetMouseRef.current, lerpFactor);
      
      // Calculate mouse velocity for more natural effects
      mouseVelocityRef.current.subVectors(mouseRef.current, prevMouseRef.current);
...
export default function Dither({
  ...
  mouseRadius = 0.3, // Increased default radius for better visual effect
  currentTheme = 'blue',
  isDarkMode = false
}) {
  ...
  // Use theme color if waveColor is not provided
  const colors = themeColors[currentTheme] || themeColors.blue;
  const themeColor = colors[0]; // Use the first color from the theme array
  const finalWaveColor = waveColor || hexToNormalizedRGB(themeColor);

  return (
    <Canvas
      className="w-full h-full relative"
      camera={{ position: [0, 0, 6] }}
      dpr={[1, Math.min(window.devicePixelRatio, 2)]} // Limit DPR for better performance
      gl={{ 
        antialias: true, 
